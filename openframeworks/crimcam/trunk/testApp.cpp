...
{	 
	...
	#else
        vidPlayer.loadMovie("crimcam_01_320x240.mov");
        vidPlayer.play();
	#endif
	...
	sampleImage.allocate( SAMPLE_WIDTH, SAMPLE_HEIGHT );
	...
	faceTracker.init( SAMPLE_WIDTH, SAMPLE_HEIGHT );
...
void testApp::update()
{
	...
	if( bNewFrame )
	{
	    ...
	    #else
            colorLargeImage.setFromPixels( vidPlayer.getPixels(), WIDTH, HEIGHT );
        #endif

		colorLargeImage.mirror( false, true );
		colorSmallImage.scaleIntoMe( colorLargeImage );
		
		displayImage	= colorLargeImage;
		sampleImage		= colorSmallImage;
		
		faceTracker.findFaces( &sampleImage );
	}
...
void testApp :: drawBlackBar ( float x, float y, float w, float h, float cx, float cy )
{
	ofFill();
	ofSetColor( 0x000000 );
	ofRect
...
void testApp :: drawBlur ( float x, float y, float w, float h, float cx, float cy )
{
	blurImage = sampleImage;
	blurImage.blur( 14 );
	...
	cx	*= sampleToSourceScale;
	cy	*= sampleToSourceScale;
...
void testApp :: drawSquares ( float x, float y, float w, float h, float cx, float cy )
{
	unsigned char *pixels = displayImage.getPixels();
	int px, py;
	int rx, ry, ri;
	int i, j;
	int r, g, b, a;
...
void testApp::keyPressed  (int key){ 
	...
	switch (key){
		case ',' :
			--crimModeIndex;
			if( crimModeIndex < 0 ) crimModeIndex = crimModeTotal - 1;
			break;
		case '.' :
			++crimModeIndex;
			if( crimModeIndex > crimModeTotal - 1 ) crimModeIndex = 0;
			break;
		case 's':
			#ifdef _USE_LIVE_VIDEO
