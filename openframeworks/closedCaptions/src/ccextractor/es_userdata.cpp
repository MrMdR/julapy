...
int user_data(struct bitstream *ustream, int udtype)
{
    ...
    if ( !memcmp(ud_header,"\x43\x43", 2 ) )
    {
        ...
        if (debug_verbose)
            printf("Reading %d%s DVD CC segments\n",
        ...
        for (int i=0; i<capcount; i++)
        {
            for (int j=0;j<2;j++)
            {
                ...
                {
                    if (debug_verbose)
                        printf("Illegal caption segment - stop here.\n");
                    maybeextracb = 0;
                    break;
                }
            }
        ...
        while ( maybeextracb && (next_u8(ustream)&0xFE) == 0xFE )
        {
            for (int j=0;j<2;j++)
            {
                ...
                {
                    if (debug_verbose)
                        printf("Illegal (extra) caption segment - stop here.\n");
                    maybeextracb = 0;
                    break;
                }
            }
    ...
    else if (ud_header[0] == 0x03)
    {
        if ((ud_header[1]&0x7F) == 0x01)
        {
            ...
            if (debug_verbose)
                printf("Reading %d SCTE 20 CC blocks\n", cc_count);
            ...
            if (debug_verbose)
                printf("Reading SCTE 20 CC blocks - done\n");
        }
    ...
    else if ( !memcmp(ud_header,"\x47\x41\x39\x34", 4 ) )
    {
        ...
        if (type_code==0x03) // CC data.
        {
            ...
            {
                if (debug_verbose)
                    printf("Reading %d HDTV CC blocks\n", cc_count);
                ...
                if (!proceed && debug_verbose)
                {
                    printf ("\rThe following payload is not properly terminated.\n");
                    dump (cc_data, cc_count*3+1);
                }
                if (debug_verbose)
                    printf("Reading %d HD CC blocks\n", cc_count);
                ...
                if (debug_verbose)
                    printf("Reading HDTV blocks - done\n");
            }
        }
    ...
    else if ( !memcmp(ud_header,"\x05\x02", 2 ) )
    {
        ...
        if (debug_verbose)
            printf("Reading Dish Network user data\n");
        ...
        if (debug_parse)
            printf("DN  ID: %02X  Count: %5u  Unknown: %04X  Pattern: %X",
        ...
        case 0x02:
            ...
            if (debug_parse)
                printf("%s", debug_608toASC( dishdata, 0) );
        ...
        case 0x05:
            ...
            if (type==0x02)
            {
                ...
                if (debug_parse)
                    printf(" - R:%02X :%s", type, debug_608toASC( dishdata, 0) );
        ...
        if (debug_verbose)
            printf("Reading Dish Network user data - done\n");
    }
    ...
    else
    {
        ...
        if (debug_verbose)
        {
            printf("Unrecognized user data:\n");
            int udatalen = ustream->end - ustream->pos;
            dump (ustream->pos, (udatalen > 128 ? 128 : udatalen) );
        }
    }

    if (debug_verbose)
        printf("User data - processed\n");
