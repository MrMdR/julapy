...
void ofxCvBlobUtil :: downSampleBlobs ( vector<ofxCvBlob> &blobsIn, vector<ofxCvBlob> &blobsOut, int numOfPoints )
{
	blobsOut.clear();
...
void ofxCvBlobUtil :: scaleBlobs ( vector<ofxCvBlob> &blobsIn, float scaleX, float scaleY )		// TODO :: need to scale the whole blob, not just points!
{
...
void ofxCvBlobUtil :: scaleBlobsToCenter ( vector<ofxCvBlob> &blobsIn, ofPoint &center, float radiusX, float radiusY )
{
...
void ofxCvBlobUtil :: translateBlobs ( vector<ofxCvBlob> &blobsIn, float x, float y )
{
	if( x == 0 && y == 0 )
		return;
