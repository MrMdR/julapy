...
public:
	...
	~EventPanel()
	{
		...
		ofRemoveListener( model->tabTypeChangeEvent,	this, &EventPanel :: tabTypeChanged );
	};
...
//		y = (int)p.y;
		...
		playUp		= (ofImage*)ofxAssets :: getInstance()->getAsset( "popup_play_a" );
		playDown	= (ofImage*)ofxAssets :: getInstance()->getAsset( "popup_play_b" );
...
//		ofRect( x + whiteBox.x, y + whiteBox.y, whiteBox.width, whiteBox.height );
		...
		if( btn.isMouseDown() || btn.isMouseOver() )
		{
			if( playDown != NULL )
				playDown->draw( x + playPoint.x, y + playPoint.y );
		}
		else
		{
			if( playUp != NULL )
				playUp->draw( x + playPoint.x, y + playPoint.y );
		}
