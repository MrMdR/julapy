...
int bufferdatatype = PES; // Can be RAW or PES
    ...
    do 
    {
        ...
        else 
        {
            ...
            if ( nextheader[3]==0xBA) 
            {
                ...
                if (falsepack) 
                {
                    printf ("Warning: Defective Pack header\n");
                }
            ...
            else if (nextheader[3]>=0xBB && nextheader[3]<=0xDF) 
            {
                ...
                if (debug_verbose)
                    printf("non Video PES (type 0x%2X) - len %u\n",
                ...
                if ( headerlen > 15000 )
                {
                    printf("Suspicious non Video PES (type 0x%2X) - len %u\n",
                           nextheader[3], headerlen);
                    printf("Do not skip over, search for next.\n");
                    headerlen = 2;
                }
...
void general_loop(void)
{
    ...
    while (!end_of_file && !processed_enough) 
    {
        ...
        position_sanity_check();
        if (clean!=NULL)
            fwrite (buffer+overlap,1,(size_t) (inbuf-overlap),clean);
        ...
        else if (bufferdatatype == RAW) // Raw two byte 608 data from DVR-MS/ASF
        {
            ...
            if (pts_set == 0)
            {
                printf("DVR-MS/ASF file without useful time stamps - count blocks.\n");
    ...
    {
        printf("\n\n\n\nATTENTION!!!!!!\n");
        printf("Processing of %s %d ended prematurely %lld < %lld, please send bug report.\n\n",
...
void rcwt_loop( void )
{
    ...
    if( !memcmp(parsebuf, "\xCC\xCC\xED", 3 ) )
    {
        ...
        {
            printf("\nRCWT header\n");
            printf("File created by %02X version %02X%02X\nFile format revision: %02X%02X\n",
    ...
    while(1)
    {
        ...
        if (debug_parse)
        {
            printf("RCWT data header FTS: %s  blocks: %u\n",
