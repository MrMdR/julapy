...
public:
	...
	void	ofxALSoundSetListenerPitch(float pitch);
	
	void	ofxALSoundSetListenerVelocity(float x, float y, float z);
	
	void	ofxALSoundSetReferenceDistance(float dist); // sets the distance after which attenuation is applied
	void	ofxALSoundSetMaxDistance(float dist); // sets the maximum distance for which attenuation is applied
