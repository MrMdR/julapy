...
void ColorPicker :: init( int width, int height )
{
	...
	colorWheel.allocate( colorWheelWidth, colorWheelHeight, GL_RGBA );
	colorWheelPixels = new unsigned char[ colorWheelWidth * colorWheelHeight * 4 ];
	...
	enableMouseEvents();
	enableKeyEvents();
	setSize( colorWheelWidth, colorWheelHeight + 40 );		// the extra height is for the color scale bar.
}
...
void ColorPicker :: updateColorScale()
{
	if( isMouseDown() )
	{
		int relX = getMouseX() - x;
		int relY = getMouseY() - y;
		
		bool lb = ( relX >= 0 );
		bool rb = ( relX < colorWheelWidth );
		bool tb = ( relY >= colorWheelHeight + 20 );
		bool bb = ( relY < colorWheelHeight + 40 );
		
		if( lb && rb && tb && bb )
		{
			colorScale = relX / (float)colorWheelWidth;
		}
...
void ColorPicker :: updateRandomWalkInterpolation()
{
	...
	randomWalkPoint.x = MIN( colorWheelWidth,  MAX( 0, randomWalkPoint.x ) );		// constrain.
	randomWalkPoint.y = MIN( colorWheelHeight, MAX( 0, randomWalkPoint.y ) );
...
void ColorPicker :: updateMouseInterpolation()
{
	if( isMouseDown() )
	{
		mousePoint.x = getMouseX() - x;
		mousePoint.y = getMouseY() - y;
		
		if( mousePoint.y < colorWheelHeight )
		{
...
void ColorPicker :: onRollOver( int x, int y )
{
	
}

void ColorPicker :: onRollOut()
{
...
void ColorPicker :: onRelease( int x, int y, int button )
{
	
}

void ColorPicker :: onReleaseOutside( int x, int y, int button )
{
	
}

void ColorPicker :: keyPressed( int key )
{
	
}

void ColorPicker :: keyReleased( int key )
{
