...
public:
	...
	inline ofxMatrix4x4() { makeIdentity(); }
	inline ofxMatrix4x4( const ofxMatrix4x4& mat) { set(mat.ptr()); }
	inline explicit ofxMatrix4x4( float const * const ptr ) { set(ptr); }
	inline explicit ofxMatrix4x4( const ofxQuat& quat ) { makeRotate(quat); }
...
//	bool operator != (const ofxMatrix4x4& m) const { return compare(m)!=0; }
	...
	inline void set(const ofxMatrix4x4& rhs) { set(rhs.ptr()); }
	...
	inline ofxVec3f getTrans() const { return ofxVec3f(_mat[3][0],_mat[3][1],_mat[3][2]); } 
...
{
	_mat[0][0] *= v.v[0]; _mat[0][1] *= v.v[0]; _mat[0][2] *= v.v[0]; _mat[0][3] *= v.v[0];
	_mat[1][0] *= v.v[1]; _mat[1][1] *= v.v[1]; _mat[1][2] *= v.v[1]; _mat[1][3] *= v.v[1];
	_mat[2][0] *= v.v[2]; _mat[2][1] *= v.v[2]; _mat[2][2] *= v.v[2]; _mat[2][3] *= v.v[2];
}

inline void ofxMatrix4x4::postMultScale( const ofxVec3f& v )
{
	_mat[0][0] *= v.v[0]; _mat[1][0] *= v.v[0]; _mat[2][0] *= v.v[0]; _mat[3][0] *= v.v[0];
	_mat[0][1] *= v.v[1]; _mat[1][1] *= v.v[1]; _mat[2][1] *= v.v[1]; _mat[3][1] *= v.v[1];
	_mat[0][2] *= v.v[2]; _mat[1][2] *= v.v[2]; _mat[2][2] *= v.v[2]; _mat[3][2] *= v.v[2];
}

// AARON METHOD
inline void ofxMatrix4x4::postMultScale( float x, float y, float z )
{
	_mat[0][0] *= x; _mat[1][0] *= x; _mat[2][0] *= x; _mat[3][0] *= x;
	_mat[0][1] *= y; _mat[1][1] *= y; _mat[2][1] *= y; _mat[3][1] *= y;
	_mat[0][2] *= z; _mat[1][2] *= z; _mat[2][2] *= z; _mat[3][2] *= z;
}
